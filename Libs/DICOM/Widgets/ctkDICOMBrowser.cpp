...
void ctkDICOMBrowser::onPatientsRightClicked(const QPoint &point)
{
  ...
  if (numPatients > 1)
  {
    ...
    selectedPatientsCount = tr(" %1 selected patients").arg(numPatients);
  }

  QString metadataString = tr("View DICOM metadata");
  if (numPatients > 1)
  {
    //: Part of the sentence "View DICOM metadata of %NUMBER selected patients"
  ...
  //: %1 refers to " %NUMBER selected patients"
  QString deleteString = tr("Delete%1").arg(selectedPatientsCount);
  QAction *deleteAction = new QAction(deleteString, patientsMenu);
  patientsMenu->addAction(deleteAction);

  //: %1 refers to " %NUMBER selected patients" (note the space character at the beginning)
  QString exportString = tr("Export%1 to file system").arg(selectedPatientsCount);
  QAction *exportAction = new QAction(exportString, patientsMenu);
  patientsMenu->addAction(exportAction);

  //: %1 refers to " %NUMBER selected patients" (note the space character at the beginning)
  QString sendString = tr("Send%1 to DICOM server").arg(selectedPatientsCount);
...
void ctkDICOMBrowser::onStudiesRightClicked(const QPoint &point)
{
  ...
  if (numStudies > 1)
  {
    //: %1 is the number of selected studies
    selectedStudiesCount = tr(" %1 selected studies").arg(numStudies);
  }

  QString metadataString = tr("View DICOM metadata");
  if (numStudies > 1)
  {
    //: Part of the sentence "View DICOM metadata of %NUMBER selected studies"
  ...
  //: %1 refers to " %NUMBER selected studies"
  QString deleteString = tr("Delete%1").arg(selectedStudiesCount);
  QAction *deleteAction = new QAction(deleteString, studiesMenu);
  studiesMenu->addAction(deleteAction);

  //: %1 refers to " %NUMBER selected studies" (note the space character at the beginning)
  QString exportString = tr("Export%1 to file system").arg(selectedStudiesCount);
  QAction *exportAction = new QAction(exportString, studiesMenu);
  studiesMenu->addAction(exportAction);

  //: %1 refers to " %NUMBER selected studies" (note the space character at the beginning)
  QString sendString = tr("Send%1 to DICOM server").arg(selectedStudiesCount);
...
void ctkDICOMBrowser::onSeriesRightClicked(const QPoint &point)
{
  ...
  if (numSeries > 1)
  {
    //: %1 is the number of selected series
    selectedSeriesCount = tr(" %1 selected series").arg(numSeries);

  }
  QString metadataString = tr("View DICOM metadata");
  if (numSeries > 1)
  {
    //: Part of the sentence "View DICOM metadata of %NUMBER selected series"
  ...
  //: %1 refers to " %NUMBER selected series" (note the space character at the beginning)
  QString exportString = tr("Export%1 to file system").arg(selectedSeriesCount);
  QAction *exportAction = new QAction(exportString, seriesMenu);
  seriesMenu->addAction(exportAction);

  //: %1 refers to " %NUMBER selected series" (note the space character at the beginning)
  QString sendString = tr("Send%1 to DICOM server").arg(selectedSeriesCount);
